#Code is wriiten by Harish Wathore, M.Tech, Department of Aerospace Engineering, Indian Institute of Technology, Kanpur
...
def run(cfg: ModulusConfig) -> None:
    # physical quantities
    nu = quantity(0.0001, "kg/(m*s)")
    rho = quantity(1.0, "kg/m^3")
    inlet_u = quantity(30.0, "m/s")
    ...
    # geometry
    channel_length = (quantity(-20, "m"), quantity(40, "m"))
    channel_width = (quantity(-20, "m"), quantity(20, "m"))
    ...
    geo = Polygon(line)
    geo = geo.scale(0.5)
    rotate_geo = geo.rotate(angle = 35 * np.pi / 18, axis = "z")

    airfoil = channel - rotate_geo
